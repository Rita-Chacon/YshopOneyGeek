...
const detallesProducto = (id, url, categoria, nombre, precio, descripcion) => {
    ...
    if(categoria == "cosmeticos"){
        cargarRelacionadoscosmeticos();
    }else if(categoria == "skinscare"){
        cargarRelacionadosskinscare();
    }
...
const cargarRelacionadoscosmeticos = () => {
    productoServices.listacosmeticos().then((data) => {
        let count = 0;
        data.forEach(({id, url, categoria, nombre, precio, descripcion}) => {
            count++;
            const nuevacosmeticos = listaRelacionados(count, id, url, categoria, nombre, precio, descripcion);
            if(nuevacosmeticos){
                productosRelacionados.appendChild(nuevacosmeticos);
            }
        });
    });
};

const cargarRelacionadosskinscare = () => {
